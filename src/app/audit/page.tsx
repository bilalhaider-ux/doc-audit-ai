...
function DashboardScreen({ fileName, fileSize, pdfFile, summary, risks, extractedText, onOpenHistory }: { fileName: string; fileSize: string; pdfFile: File | null, summary: string[], risks: any[], extractedText: string, onOpenHistory: () => void }) {
  ...
  return (
    <div className="h-screen bg-surface-container-low flex overflow-hidden animate-[fade-in_0.5s_ease-out]">
      {/* Sidebar */}
      <aside className="w-16 md:w-20 bg-surface border-r border-outline-variant flex flex-col items-center py-6 gap-8 z-20">
        <Link href="/">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        {/* Top Header */}
        <header className="h-14 bg-surface border-b border-outline-variant flex items-center justify-between px-5 z-10">
          <div className="flex items-center gap-3 min-w-0">
            <span className="material-symbols-outlined text-primary text-[24px]" style={{ fontVariationSettings: "'FILL' 1" }}>description</span>
            <div className="flex flex-col truncate">
              <h1 className="text-[15px] font-semibold text-on-surface truncate leading-tight">{fileName}</h1>
              <span className="text-[11px] text-on-surface-variant leading-tight">Processed just now • {fileSize}</span>
            </div>
            <div className="hidden md:flex ml-3 px-2.5 py-1 bg-secondary-container/20 rounded-full items-center gap-1.5">
          ...
          <div className="flex items-center gap-2 shrink-0">
            <ThemeToggle />
            <div className="w-px h-5 bg-outline-variant/50 hidden md:block mx-1"></div>
            <button 
              onClick={handleExport}
              className="hidden md:flex items-center gap-1.5 px-4 py-1.5 border border-outline-variant rounded-full text-on-surface hover:bg-surface-container-high transition-colors text-[13px] font-medium"
            >
            ...
            <div>
              <button 
                ...
                className="flex items-center gap-1.5 px-4 py-1.5 bg-primary text-on-primary rounded-full hover:opacity-90 transition-all text-[13px] font-medium shadow-sm active:scale-95"
              >
              ...
              {showShareMenu && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                  <div className="absolute inset-0 bg-surface-on/50 backdrop-blur-sm" onClick={() => setShowShareMenu(false)}></div>
                  <div className="relative w-full max-w-sm bg-surface border border-outline-variant rounded-3xl shadow-2xl overflow-hidden animate-[fade-in_0.3s_ease-out]">
        ...
        <main className="flex-1 flex flex-col md:flex-row overflow-hidden relative">
          ...
          {/* Right AI Panel */}
          <section className="w-full md:w-[400px] border-l border-outline-variant bg-surface flex flex-col shadow-[-4px_0_24px_rgba(0,0,0,0.02)] z-10 shrink-0">
            {/* Panel Tabs */}
            <div className="flex border-b border-outline-variant px-2 pt-2 bg-surface">
              <TabButton
                label="Summary"
                icon="notes"
            ...
            {/* Panel Content Area */}
            <div className="flex-1 overflow-y-auto custom-scrollbar p-4 bg-surface-container-low">
              {activeTab === "risks" && (
            ...
            {activeTab === "chat" && (
              <div className="p-3 bg-surface border-t border-outline-variant z-20">
                <form 
